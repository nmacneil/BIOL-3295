...
#First, clear all the old stuff in R. Always put this at the top of your code.
rm(list=ls())
...
# 3) R has a huge user community. You can most likely find solutions to any problems you have by typing your question about R in google. We expect you to first try to solve your own problems by troubleshooting using google or R help (help()). 
...
#If you have data, you can import it into R and run calculations on your data. We will import a sample of the data used in Leroux & Kerr 2013 Land Development in and around protected areas at the wilderness frontier. Conservation Biology 27:166-176.
...
#In the above expression -str- is a function and it is applied to the object in parenthesis - the data

#This outputs the first 6 lines of your data
head(MyData)

#...and the last 6 lines of your data
tail(MyData)
...
#Intuitively, the primary function for plotting is called 'plot'. Let's see what arguments 'plot' takes so we know how to make our figure of x vs y
help(plot)
#Type help(par) to find out everything you can do with this simple plotting function. par describes the different arguments you can provide to plot to change the appearance of your plot.
plot(x,y)
...
# DATA FRAMES
#############
#Being able to create data frames or tables is a fundamental skill you should practice
#Data frames are the best way to store data in R. It is easy to work with (ie run analyses) on data frames
#Create a data frame called "DataData". The data frame will have three columns named -> Column1, Column2, and Column3. Each column will have 4 rows of data.
DataData <- data.frame(Column1=c(2,4,6,8), Column2=seq(1:4), Column3=c(8,6,4,2))
DataData
#you can look up what the function c() does
#Now, let's add a column named 'Column4' to the data frame. You can call columns from data frames by listing them after a $ as below.
DataData$Column4 <- c(0.1,0.2,0.3,0.4)
DataData
#Let's multiply Column1 x Column4 and put the results in a new column called Column5
